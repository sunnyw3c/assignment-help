...
<div class="relative group">
    ...
    <!-- Main Container -->
    <div class="relative bg-slate-900/75 backdrop-blur-xl rounded-3xl p-6 md:p-10 shadow-[0_24px_60px_-20px_rgba(0,0,0,0.7)] border border-white/10">
        
        <!-- Header -->
        <div class="text-center mb-10">
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-400/30 mb-4">
                <span class="relative flex h-2.5 w-2.5">
                  <span class="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
                  <span class="relative inline-flex rounded-full h-2.5 w-2.5 bg-blue-500"></span>
                </span>
                <span class="text-blue-200 font-bold text-xs uppercase tracking-wider">Live Estimate</span>
            </div>
            <h3 class="text-3xl font-black text-white mb-2">Calculate Your Price</h3>
            <p class="text-slate-300">Get an instant quote with our transparent pricing tool.</p>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-8 lg:gap-12">
            ...
            <div class="space-y-8">
                <!-- Page Counter -->
                <div>
                    <label class="block text-sm font-bold text-slate-200 mb-3 uppercase tracking-wide">Paper Length</label>
                    <div class="flex items-center gap-4 bg-slate-800/80 p-2 rounded-2xl border border-slate-700/70 shadow-sm">
                        <button type="button" wire:click="decrementPages"
                            class="w-12 h-12 flex items-center justify-center rounded-xl bg-slate-700/70 text-slate-200 hover:bg-red-500/15 hover:text-red-300 transition-colors duration-200">
                        ...
                        <div class="flex-1 text-center">
                            <div class="text-2xl font-black text-white leading-none">{{ $pages }}</div>
                            <div class="text-xs font-bold text-slate-400 uppercase tracking-wider mt-1">{{ $pages === 1 ? 'Page' : 'Pages' }}</div>
                        </div>

                        <button type="button" wire:click="incrementPages"
                            class="w-12 h-12 flex items-center justify-center rounded-xl bg-slate-700/70 text-slate-200 hover:bg-green-500/15 hover:text-green-300 transition-colors duration-200">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"/></svg>
                        </button>
                    </div>
                    <div class="text-center mt-2 text-xs font-medium text-slate-400">
                ...
                <!-- Academic Level -->
                <div>
                    <label class="block text-sm font-bold text-slate-200 mb-3 uppercase tracking-wide">Academic Level</label>
                    <div class="relative">
                        <select wire:model.live="academicLevel"
                            class="w-full appearance-none bg-slate-800/80 border border-slate-700 rounded-xl px-4 py-3.5 pr-10 font-bold text-slate-100 focus:outline-none focus:border-purple-400 focus:ring-4 focus:ring-purple-500/20 transition-all cursor-pointer hover:border-purple-500/60">
                ...
                <!-- Deadline -->
                <div>
                    <label class="block text-sm font-bold text-slate-200 mb-3 uppercase tracking-wide">Deadline</label>
                    <div class="relative">
                        <select wire:model.live="deadline"
                            class="w-full appearance-none bg-slate-800/80 border border-slate-700 rounded-xl px-4 py-3.5 pr-10 font-bold text-slate-100 focus:outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-500/20 transition-all cursor-pointer hover:border-blue-500/60">
            ...
            <div class="flex flex-col justify-between bg-slate-800/70 rounded-2xl p-6 md:p-8 shadow-sm border border-slate-700/70 relative overflow-hidden">
                ...
                <div>
                    <label class="block text-sm font-bold text-slate-400 mb-4 uppercase tracking-wide">Estimated Total</label>
                    ...
                    <div class="flex items-center gap-3 mb-8">
                        <span class="text-lg font-bold text-slate-500 line-through decoration-2 decoration-red-400/50">${{ number_format($this->totalPrice, 2) }}</span>
                        <span class="px-2.5 py-1 rounded-lg bg-emerald-500/20 border border-emerald-400/40 text-emerald-300 text-xs font-black uppercase tracking-wide">
                    ...
                    <div class="space-y-3 mb-8">
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-slate-400 font-medium">Price per page</span>
                            <span class="font-bold text-slate-100">${{ number_format($this->pricePerPage, 2) }}</span>
                        </div>
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-slate-400 font-medium">Seasonal Discount</span>
                            <span class="font-bold text-emerald-300">-{{ number_format($this->discount * 100, 0) }}%</span>
                        </div>
                        @if ($this->volumeDiscount > 0)
                        <div class="flex justify-between items-center text-sm">
                            <span class="text-slate-400 font-medium">Volume Discount</span>
                            <span class="font-bold text-emerald-300">-{{ number_format($this->volumeDiscount * 100, 0) }}%</span>
                        </div>
                        @endif
                    </div>
                </div>
        ...
        <!-- Features Footer -->
        <div class="mt-8 pt-6 border-t border-white/10 flex flex-wrap justify-center gap-6 md:gap-12 text-sm text-slate-300 font-medium">
